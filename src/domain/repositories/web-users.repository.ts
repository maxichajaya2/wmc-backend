...
export class WebUsersRepository {
  ...
  async create(preRegisterDto: CreateWebUserDto) {
    console.info('🧩 Datos recibidos en create():', preRegisterDto);
    ...
    try {
      const newUser = await this.repository.save(user);
      console.info('✅ Usuario creado correctamente:', newUser);
      return newUser;
    } catch (error) {
      console.error('❌ Error al guardar usuario en BD:', error);
      console.error('📦 Datos que causaron el error:', user);

      // Retornar un error más visible en desarrollo
      throw new InternalServerErrorException({
        message: 'Error creating web user',
        reason: error?.message ?? 'Unknown error',
        detail: error?.detail ?? null, // PostgreSQL y MySQL suelen poner info aquí
        user,
      });
    }
  }
