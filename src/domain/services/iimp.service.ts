...
export class IimpService {
    ...
    async register(webUser: WebUser): Promise<ServiceResponse<{ personId: number } | null>> {
        console.log('IimpService - register :: Registering web user', webUser);
        const { id_event, siecode_event } = await this.parametersRepository.getEventParameters();
    ...
    async credentialCreate(webUser: WebUser): Promise<ServiceResponse<{ password: string, decrypted_password: string } | null>> {
        ...
        console.log(`IimpService - credentialCreate :: ${JSON.stringify({ status })}`);
        console.log(`IimpService - credentialCreate :: ${JSON.stringify({ data })}`);
    ...
    async verifyCredentials(webUser: WebUser): Promise<ServiceResponse<ValidateSieResponse>> {
