...
const EXPIRATION_TIME_IN_SECONDS = process.env.OTP_EXPIRATION_TIME_IN_SECONDS
  ? Number(process.env.OTP_EXPIRATION_TIME_IN_SECONDS)
  : 300;
...
export class UserTokensService {
  ...
  async sendResetPasswordOtp(email: string) {
    ...
    const expiresAt = new Date(
      now.getTime() + 1000 * EXPIRATION_TIME_IN_SECONDS,
    );
