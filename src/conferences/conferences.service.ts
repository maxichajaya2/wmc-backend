...
export class ConferencesService {
  ...
  async create(createConferenceDto: CreateConferenceDto) {
    ...
    const newConference =
      await this.conferenceRepository.repository.save(conference);
  ...
  private toJson(c: Conference) {
    ...
    return {
      ...
      conferenceSpeakers: c.conferenceSpeakers.map((cs) => {
        const country = cs.speaker?.countryCode
          ? this.countriesService.getCountry(cs.speaker.countryCode)
          : null;
  ...
  async findAll(
    { mode = 'public', speakerId } = { mode: 'public', speakerId: undefined },
  ): Promise<any> {
    if (speakerId) {
    ...
    if (mode === 'public') {
      ...
      conferences.forEach((conference) => {
        ...
        const startPeruDate =
          DateTime.fromISO(startDate).setZone('America/Lima');
      ...
      Object.entries(response).forEach(([dayDate, conferencesList]) => {
        ...
        for (const conference of conferencesList) {
          ...
          const roomName =
            conference.room.nameEn + '&&&' + conference.room.nameEs;
          console.log({ roomName });
          const claveExistente = Object.keys(response2[dayDate]).find(
            (hora) => {
              console.log({ dayDate, hora, roomName });
              const roomKeys = Object.keys(response2[dayDate][hora]);
              const conferencesInHour = roomKeys.flatMap(
                (roomKey) => response2[dayDate][hora][roomKey],
              );
          ...
          if (claveExistente) {
            ...
            const existsElements =
              conferencesInRoom && conferencesInRoom.length > 0;
            if (existsElements) {
          ...
          console.log({
            response2: JSON.stringify(this.shortResponse2(response2), null, 2),
          });
        }
      });
      return response2;
