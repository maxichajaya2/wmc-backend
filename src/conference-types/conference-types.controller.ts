...
export class ConferenceTypesController {
  constructor(
    private readonly conferenceTypesService: ConferenceTypesService,
  ) {}
  ...
  findAll(@Query('onlyActive') onlyActive: string) {
    return this.conferenceTypesService.findAll({
  ...
  findOne(@Param('id') id: string, @Query('onlyActive') onlyActive: string) {
    return this.conferenceTypesService.findOne(+id, {
      onlyActive: onlyActive === 'true',
    });
  }
  ...
  update(
    @Param('id') id: string,
    @Body() updateConferenceTypeDto: UpdateConferenceTypeDto,
  ) {
